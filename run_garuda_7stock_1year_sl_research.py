...
def main():
    ...
    print(
        "SL modes       : ORB | 50% ORB | VWAP-MIDDLE"
    )
    ...
    for symbol in SYMBOLS:
        ...
        for mode in sl_modes:
            ...
            # VWAP invalid risk is a scientific failure,
            # not a reason to silently shrink the universe.
            if invalid_count and mode == "VWAP_MIDDLE":

                raise RuntimeError(
                    f"{symbol}: VWAP-MIDDLE produced "
                    f"{invalid_count} invalid-risk entries. "
                    "No apples-to-apples result accepted."
                )
