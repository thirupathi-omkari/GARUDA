...
# ============================================================
# GARUDA — 7-STOCK / 1-YEAR ORB+VWAP SL × TARGET RESEARCH
#
...
# Target-finalization ladder.
# All SL modes are tested against the exact same frozen entries.
TARGET_LADDER_R = [
    1.00,
    1.25,
    1.50,
...
def main():
    ...
    print(
        f"Target ladder  : 1.00R, 1.25R, 1.50R, 1.75R, 2.00R, 2.25R, 2.50R, 2.75R, 3.00R"
    )
    ...
    # --------------------------------------------------------
    # Replay every frozen universe through:
    #
    #   3 SL modes × 9 target levels = 27 combinations
    #
    # The frozen entries are NEVER regenerated when target or
    # SL changes. This is the key apples-to-apples constraint.
    # --------------------------------------------------------
    sl_modes = [
        "ORB",
        "ORB_50",
    ...
    for symbol in SYMBOLS:
        ...
        for target_r in TARGET_LADDER_R:
            ...
            for mode in sl_modes:
                ...
                if len(trades) != expected_valid:
                    ...
                    raise RuntimeError(
                        f"{symbol} {mode} "
                        f"{target_r:.2f}R: replay count "
                        f"{len(trades)} != expected valid "
                        f"entries {expected_valid}; "
                        f"frozen universe={len(entries)}"
                    )
    ...
    summary_path = (
        RESEARCH_DIR
        / "garuda_7stock_1year_sl_target_matrix_summary.csv"
    )
    ...
    def coverage_entry_count(
        entry_map,
        symbols,
    ):
        return sum(
            len(entry_map[symbol])
            for symbol in symbols
        )

    # --------------------------------------------------------
    # Combined 7-stock aggregate for every SL × target cell.
    # --------------------------------------------------------
    aggregate_rows = []
            ...
            aggregate_rows.append(
                {
                    ...
                    "INVALID_RISK_ENTRIES":
                        int(
                            sum(
                                row["INVALID_RISK_ENTRIES"]
                                for row in symbol_summary_rows
                                if row["SL_MODE"] == mode
                                and row["TARGET_R"] == target_r
                            )
                        ),
                    "TRADES":
                        len(mode_df),
    ...
    print("=" * 140)
    print(
        "GARUDA — 7-STOCK / 1-YEAR SL × TARGET MATRIX"
    )
    print("=" * 140)

    print(
        "Cells tested : "
        f"{len(sl_modes)} SL modes × "
        f"{len(TARGET_LADDER_R)} targets = "
        f"{len(sl_modes) * len(TARGET_LADDER_R)}"
    )

    print(
        "Frozen universe is identical across every valid cell."
    )

    print()
    print(
        "7-STOCK AGGREGATE MATRIX"
    )
    print("=" * 140)
    ...
    print(
        "Any structurally invalid SL cases are reported and excluded "
        "only from that SL mode's performance statistics; no fallback "
